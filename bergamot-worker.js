...
const MODEL_REGISTRY =
  "https://storage.googleapis.com/bergamot-models-sandbox/0.3.1/registry.json";
const MODEL_ROOT_URL =
  "https://storage.googleapis.com/bergamot-models-sandbox/0.3.1";
...
onmessage = async function(e) {
  ...
  if (command === "import") {
    ...
    try {
      ...
      log(
        `Model '${from}${to}' successfully constructed. Time taken: ${(Date.now() -
          start) /
          1000} secs`
      );
    ...
    try {
      ...
      log(
        `Translation '${from}${to}' Successful. Speed: ${Math.round(
          inputWordCount / secs
        )} WPS (${inputWordCount} words in ${secs} secs)`
      );
    } catch (error) {
      log(`Error: ${error.message}`);
    }
    log(`'${command}' command done, Posting message back to main script`);
    postMessage([`${command}_reply`, result, id]);
  }
};
...
const constructTranslationService = async () => {
  if (!translationService) {
    ...
    log(
      `Creating Translation Service with config: ${translationServiceConfig}`
    );
...
const constructTranslationModel = async (from, to) => {
  ...
  if (_isPivotingRequired(from, to)) {
    ...
    await Promise.all([
      _constructTranslationModelHelper(languagePairSrcToPivot),
...
const translate = (from, to, input, translateOptions) => {
  ...
  try {
    ...
    const listTranslatedTextSentences = _parseTranslatedTextSentences(
      vectorResponse
    );
...
const _prepareAlignedMemoryFromBuffer = async (buffer, alignmentSize) => {
  ...
  var alignedMemory = new Module.AlignedMemory(
...
`;
  ...
  const promises = [];
  fileInfo
    .filter(file => modelRegistry[languagePair].hasOwnProperty(file.type))
    .map(file => {
  ...
  if (alignedMemories.length === fileInfo.length) {
    ...
    translationModel = new Module.TranslationModel(
      modelConfig,
      alignedMemories[0],
      alignedMemories[1],
